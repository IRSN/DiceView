...
roots = function (f, vectorized=FALSE, interval, maxerror_f = 1e-07, split = "seq", split.size = 11,
    ...
    I.roots = .lapply(I.list, function(I) {
        r = NULL
        try({
            r = root(f, lower = I[1], upper = I[2], # = I[3], f_upper = I[4],
...
#'
#' r = mesh_roots(f = function(x) (0.25+x[1])^2+(0.5+x[2])^2 - .25,
#' intervals=matrix(c(-1,1,-1,1),nrow=2), mesh.size=5)
#' plot(r,xlim=c(-1,1),ylim=c(-1,1))
#'
#' r = mesh_roots(function(x) (0.5+x[1])^2+(-0.5+x[2])^2+(0.+x[3])^2 - .5,
#'                mesh.sizes = 11,
...
mesh_roots = function (f, vectorized = FALSE, intervals, mesh = "seq", mesh.sizes = 11,
  ...
  if (d == 1)
    return(roots(f, vectorized = vectorized, interval = intervals, split = mesh, split.size = mesh.sizes,
                 maxerror_f = maxerror_f, tol = tol, ...))
  ...
  r = do.call(rbind,mclapply(X = 1:nrow(ridges), FUN = function(ridge_i,...) {
